namespace Favorites
{
	...
	public class UnityGUIList<T> where T : IDraggable
	{
		...
		private Color folderDropMarkerColor = new Color( 67f / 255, 160f / 255, 71f / 255 );
			...
			ListEventDiscriminator listEventDiscriminator )
		{
			...
			this.listEventDiscriminator.WhenDroppedIntoItem += eventsHandler.OnExternalItemsWereDroppedIntoFolder;
		...
		public bool DoList( GUIEvent e )
		{
			...
			for ( int i = firstInWindow; i <= lastInWindow; i++ )
			{
				...
				Rect rawItemBounds = drawListItem( item );
				Rect itemBounds = gui.BoundsCorrection( rawItemBounds );
				...
				Rect insertionBounds = itemBounds;
				insertionBounds.y -= insertionBounds.height / 2;
				bool isHoveringInsertionPoint = insertionBounds.Contains( mousePositionInScroll ) && IsMouseOrDrag( e );
				Rect itemDropBounds = itemBounds;
				itemDropBounds.y += itemDropBounds.height * 0.25f;
				itemDropBounds.height *= 0.5f;
				bool canDropIntoItem =
					item.Value.CanAcceptExternalAssetDrop
					&& eventsHandler.CanDropExternalItemsIntoFolder( i );
				bool isHoveringItemDropTarget =
					canDropIntoItem
					&& itemDropBounds.Contains( mousePositionInScroll )
					&& IsMouseOrDrag( e );

				// Save the drop marker idx for the next draw
				// Necessary to do it this way to avoid layout errors with imgui
				if ( isHoveringItemDropTarget )
				{
					nextDropMarkerIdx = int.MinValue;

					if ( e.type == EventType.DragUpdated )
					{
						eventsHandler.OnExternalFolderDragUpdated();
						gui.RectMarker( rawItemBounds, folderDropMarkerColor );
					}

					if ( e.type == EventType.DragPerform )
						listEventDiscriminator.MouseDropped_IntoItem( i );
				}
				else if ( isHoveringInsertionPoint )
				{
					nextDropMarkerIdx = i;
	...
	public interface IDraggable
	{
		...
		bool CanAcceptExternalAssetDrop { get; }
	}
}

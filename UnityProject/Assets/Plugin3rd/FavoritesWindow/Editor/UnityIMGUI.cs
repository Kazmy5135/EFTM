namespace Favorites
{
	...
	public class UnityIMGUI : IGUI
	{
		...
		private Rect? rectMarkerPosition;
		private Color rectMarkerColor;
		...
		public void OnGUIBegun()
		{
			...
			rectMarkerPosition = null;
		}

		public void OnGUIEnded()
		{
			if ( rectMarkerPosition != null && Event.current.type == EventType.Repaint )
			{
				var position = rectMarkerPosition.Value;
				var color = rectMarkerColor;
		...
		public void RectMarker( Rect rect, Color color )
		{
			rect.position -= scrollPosition;
			rect = BoundsCorrection( rect );
			rect.x += 2;
			rect.width = Mathf.Max( 0, rect.width - 4 );
			this.rectMarkerPosition = rect;
			this.rectMarkerColor = color;
		}

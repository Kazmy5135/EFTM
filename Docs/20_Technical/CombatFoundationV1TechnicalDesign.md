...
> 当前契约：2026-09-08 设计者批准 008 双侧换边，随后体验确认取消预转头/回看，改为面向通道直接横移 0.80s；随后批准移动侧探、侧倾及压低（见 008/MovingLeanProposal.md）。增量正文见本文件“008 双侧换边已批准增量契约”，取代基础层中的单掩体、仅假 Peek 采样、移动非目标和统一失焦返回假设；其余基础契约保留。以下“当前事实”是首次基线建立时的历史事实。直视通道实现及本次修订前基线已提交推送为 `49ce351`；本轮移动侧探增量按最新批准记录同步，原 `60d1c13` 不包含本扩展。
...
- 点击后首个有效更新即产生位移；移动全程 PlayerRoot 使用 CoverSideRig.ForwardReference；Camera 观察前向沿该方向，在其前向轴上叠加来源侧侧倾，相对身体增加向行进侧的头部偏移和轻微压低。由同一快照进度分别采样头部/侧倾曲线，不受敌人或旧情报影响。两侧 HiddenPose 与 PlayerAnchor 朝向须与通道参考一致，启动校验不一致则报错，避免起止跳转。
...
| 移动镜头朝向 | 前向固定通道，叠加最多 4° 来源侧侧倾 | 头部偏移峰值 0.12s、侧倾峰值 0.16s，0.34s 后减弱，0.80s 归零；不追踪敌人或旧情报 |
| 头部侧探/压低 | 0.08m / 0.02m | 头部曲线进度 0、0.15、0.425、0.75、1 对应权重 0、1、1、0.4、0；侧倾峰值进度改为 0.20；分段零切线平滑插值 |
| 额外自动观察/周期性晃动 | 0s / 关闭 | 不增加自动 Peek；合成相机行进位置须单调，起止附加量为零 |
...
- 镜头以包含所有 near clip 平面角点的保守球包络和合成后路径段扫掠检查与墙体的间距；侧倾不影响此球的包络保证。运行时和 Editor 采用同一 EvaluateMotion，启动以 128 段验证头部曲线、扫掠和行进单调性；清单变更后重新验证，不能依靠提高 near clip 隐藏穿模。
...
- PlayMode：左右两侧、五点位、完整连续路径、相机侧倾、首帧横移、前向沿通道、受控侧倾及头部偏移、起止归零、跨侧旧世界锚点与两侧射线、无自动 Peek、30/60Hz 的 0.80s 行为和终点帧时序。

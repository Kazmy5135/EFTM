...
- [x] 建立五点位、掩体和 20 点等权 VisibilitySampleRig。
- [x] 实现无分配遮挡评估和 10% 阈值。
- [x] 实现旧位置快照、黄色双 Pass 剪影和独立 Layer。
- [x] 实现一次性旧位置自动预瞄。

## U4 射击与后坐力

- [x] 实现中心射线、提前 armed 与 108ms 连射。
- [x] 实现中央/开火指针调瞄。
- [x] 实现首发、爬升、稳定和随机水平后坐力。
- [x] 预热并池化弹道、音频和命中反馈。
- [x] 验证射线、可见性与连续场景 Step 热路径无持续托管分配；设备端整帧 Profiler 待 U5。

## U5 验证与回传

- [x] 运行框架校验、EditMode、PlayMode 和 Windows 开发构建验证（Android 单列）。
- [ ] 生成 Android 开发构建并完成真机验证。
- [ ] 按统一脚本完成 H5/Unity 行为对照。
- [x] 更新验证、项目状态和追踪登记，并准备设备验收脚本。
- [ ] 经设计者验收后关闭工作项。
